            ...
            .call( b -> b
                ...
                .call( c -> c.to( Actors.GITHUB )
                    ...
                    .response( ApiMessage.response(
                        "workflow_runs[0].id", 11111 )
                        .masking( Unpredictables.RNG,
                            m -> m.replace( "workflow_runs[0].id", "_masked_" ) ) ) )
                .call( c -> c.to( Actors.GITHUB )
                    .tags( Tags.add( "artifacts" ) )
                    .request( ApiMessage.request(
                        "/repos/therealryan/bowlby/actions/runs/" + PathVars.RUN_ID
                            + "/artifacts" )
                        .set( PathVars.RUN_ID, 11111 )
                        .masking( Unpredictables.RNG, m -> m
                            .replace( PathVars.RUN_ID, "_masked_" ) ) )
                    .response( ApiMessage.response(
                        "artifacts[0].id", 22222,
                        "artifacts[0].name", "artifact_alpha",
                        "artifacts[1].id", 33333,
                        "artifacts[1].name", "artifact_beta" )
                        .masking( Unpredictables.RNG,
                            m -> m
                                .replace( "artifacts[0].id", "_masked_" )
                                .replace( "artifacts[1].id", "_masked_" ) ) ) )
        ...
        .call( a -> a
            ...
            .call( b -> b.to( Actors.BOWLBY )
                ...
                .response( HttpMessage.redirectResponse(
                    "/artifacts/therealryan/bowlby/" + 22222 + "/" )
                    .set( HttpMsg.header( "cache-control" ),
                        "public; immutable; max-age=599" ) ) )
            .call( b -> b.to( Actors.BOWLBY )
                .tags( Tags.add( "artifact" ) )
                .request( HttpMessage.chromeRequest( "GET",
                    "/artifacts/therealryan/bowlby/" + PathVars.ALPHA_ID + "/" )
                    .set( PathVars.ALPHA_ID, 22222 ) )
                .call( c -> c.to( Actors.GITHUB )
                    .request( ApiMessage.request(
                        "/repos/therealryan/bowlby/actions/artifacts/"
                            + PathVars.ALPHA_ID + "/zip" )
                        .set( PathVars.ALPHA_ID, 22222 )
                        .masking( Unpredictables.RNG, m -> m
                            .replace( PathVars.ALPHA_ID, "_masked_" ) ) )
